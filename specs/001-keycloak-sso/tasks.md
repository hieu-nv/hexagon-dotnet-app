...
[x] T002 Create authentication configuration structure in `src/App.Core/Authentication/` directory with empty placeholder files
[x] T003 Create authorization configuration structure in `src/App.Core/Authorization/` directory with empty placeholder files
[x] T004 Update .github/copilot-instructions.md with OAuth2/OpenID Connect patterns and conventions for this feature
[x] T005 Create Keycloak configuration directory at `/scripts/keycloak/` for realm and client configurations
...
[x] T007 [P] Create `IAuthorizationPolicyProvider` port interface in `src/App.Core/Authorization/Ports/IAuthorizationPolicyProvider.cs` with methods:
...
[x] T010 [P] Create `OpenIdConnectConfiguration` value object in `src/App.Core/Authentication/OpenIdConnectConfiguration.cs` with properties: Enabled, Authority, ClientId, ClientSecret, Scopes, RequiredClaims
...
[x] T012 Create `AuthorizationService` domain service in `src/App.Core/Authorization/AuthorizationService.cs` that:
...
[x] T013 Create `AuthenticationExtensions.cs` in `src/App.Core/Authentication/` with:
...
[x] T014 Create `AuthorizationExtensions.cs` in `src/App.Core/Authorization/` with:
...
[x] T015 [P] [US1] Create integration test in `test/App.Core.Tests/Authentication/KeycloakSetupTests.cs` verifying:
...
[x] T016 [P] [US1] Create Keycloak realm configuration file in `scripts/keycloak/realm-export.json` with:
  - Realm name: "hexagon"
  - Event logs enabled
  - SMTP configuration for email (optional)
  - Password policy: minimum 8 characters, requires uppercase and number

[x] T017 [P] [US1] Create Keycloak OAuth2 client configuration in `scripts/keycloak/client-config.json` with:
...
[x] T018 [P] [US1] Create Keycloak test users setup file in `scripts/keycloak/users-roles.json` with:
...
[x] T021 [US1] Create README documenting Keycloak setup in `docs/KEYCLOAK_SETUP.md`:
...
[x] T022 [P] [US2] Create unit test in `test/App.Core.Tests/Authentication/OpenIdConnectConfigurationTests.cs` verifying:
  - Configuration loads correctly from appsettings.json
  - Validates required properties (Authority, ClientId, ClientSecret)
  - Handles missing configuration gracefully

[x] T023 [P] [US2] Create integration test in `test/App.Api.Tests/Authentication/OpenIdConnectMiddlewareTests.cs` verifying:
...
[x] T025 [US2] Create `OpenIdConnectConfiguration.cs` loader in `src/App.Api/Configuration/` that:
...
[x] T026 [US2] Implement OpenID Connect middleware registration in `src/App.Api/Authentication/OpenIdConnectExtensions.cs`:
...
[x] T028 [US2] Create test endpoint `GET /auth/status` in `src/App.Api/Authentication/AuthStatusEndpoints.cs` for manual verification:
...
[x] T029 [US2] Add comprehensive logging for OAuth2 events to `src/App.Api/Logging/`:
...
[x] T033 [US3] Create constants file `src/App.Core/Authentication/OpenIdConnectClaims.cs` defining:
...
[x] T034 [US3] Create `AuthServiceExtensions.cs` in `src/App.Core/Authentication/` that:
...
[x] T035 [US3] Update `EntityFrameworkCore.OpenIdConnectTokenHandler.cs` (new file) to:
...
[x] T036 [US3] Create `AuthenticationEventHandler.cs` in `src/App.Api/Authentication/` that:
...
[x] T037 [US3] Create sample endpoint handler `src/App.Api/Authentication/AuthInfoEndpoints.cs`:
...
[x] T038 [P] [US4] Create unit test in `test/App.Core.Tests/Authorization/AuthorizationPolicyTests.cs` verifying:
...
[x] T039 [P] [US4] Create unit test in `test/App.Core.Tests/Authorization/AuthorizationServiceTests.cs` verifying:
...
[x] T040 [P] [US4] Create `AuthorizationPoliciesRegistry.cs` in `src/App.Core/Authorization/` defining standard policies:
...
[x] T041 [US4] Implement `DefaultAuthorizationPolicyProvider` adapter in `src/App.Data/Authorization/DefaultAuthorizationPolicyProvider.cs`:
...
[x] T042 [US4] Update `AuthorizationService` in `src/App.Core/Authorization/AuthorizationService.cs`:
...
[x] T043 [US4] Create authorization helpers in `src/App.Core/Authorization/AuthorizationHelpers.cs`:
...
[x] T044 [US4] Create documentation in `docs/AUTHORIZATION_POLICIES.md`:
...
[x] T045 [P] [US5] Create integration test in `test/App.Api.Tests/Authentication/AuthorizedEndpointsTests.cs` verifying:
...
[x] T046 [P] [US5] Create contract test in `test/App.Api.Tests/Authentication/AuthenticationContractTests.cs` verifying:
...
[x] T047 [P] [US5] Create authorization middleware helper in `src/App.Api/Authentication/AuthorizationMiddleware.cs`:
...
[x] T048 [US5] Update Todo endpoints in `src/App.Api/Todo/TodoEndpoints.cs`:
...
[x] T049 [US5] Create example protected endpoint `src/App.Api/Admin/AdminEndpoints.cs`:
...
[x] T050 [US5] Update Swagger/OpenAPI configuration in `src/App.Api/Program.cs`:
...
[x] T052 [US5] Add authorization decision logging to all protected endpoints in `src/App.Api/Authentication/AuthorizationLogging.cs`:
...
[x] T053 [P] [US6] Create integration test in `test/App.Api.Tests/Authentication/LogoutTests.cs` verifying:
...
[x] T054 [P] [US6] Create logout endpoint handler in `src/App.Api/Authentication/AuthLogoutEndpoints.cs`:
...
[x] T055 [US6] Implement logout service `src/App.Core/Authentication/LogoutService.cs`:
...
[x] T056 [US6] Create logout extension in `src/App.Core/Authentication/LogoutExtensions.cs`:
...
[x] T057 [US6] Create post-logout redirect page `src/App.Api/Static/logout-success.html` (or Razor page):
  - Shown after logout completes
  - Friendly message confirming logout
  - Link to login page for reconvening authentication

[x] T058 [US6] Update documentation in `docs/KEYCLOAK_SETUP.md` with logout flow:
...
[x] T059 Create comprehensive OAuth2/OpenID Connect architecture guide in `docs/OAUTH2_ARCHITECTURE.md`:
...
[x] T060 Create OAuth2 troubleshooting guide in `docs/OAUTH2_TROUBLESHOOTING.md`:
...
[x] T061 [P] Create OAuth2 integration tests in `test/App.Api.Tests/Authentication/OAuth2IntegrationTests.cs`:
  - End-to-end authentication flow with real Keycloak (Docker container)
  - Token refresh flow (if implemented)
  - Session timeout and re-authentication
  - Token expiration handling

[x] T062 [P] Create performance tests in `test/App.Core.Tests/Authentication/AuthenticationPerformanceTests.cs`:
...
[x] T063 Create OAuth2 security checklist in `docs/OAUTH2_SECURITY.md`:
...
[x] T064 Create OAuth2 configuration reference in `docs/OAUTH2_CONFIG_REFERENCE.md`:
...
[x] T065 Add OAuth2 code examples in `docs/OAUTH2_EXAMPLES.md`:
...
[x] T066 Create OpenAPI/Swagger documentation updates:
...
[x] T067 [P] Handle edge cases in `src/App.Core/Authentication/` and `src/App.Api/Authentication/`:
...
[x] T068 Review code against project standards:
...
[x] T069 Create unit test summary in `specs/001-keycloak-sso/checklists/test-coverage.md`:
...
[x] T070 Update project README in `README.md` with OAuth2:
  - Add "Authentication" section
  - Quick start for Keycloak setup
  - Links to documentation
  - Feature status badge (OAuth2 implemented)

[x] T071 Create GitHub issue templates for OAuth2 bugs/features in `.github/ISSUE_TEMPLATE/`:
